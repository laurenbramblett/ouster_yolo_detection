#!/home/administrator/yolo-env/Python-3.8.10/yolo-env/bin/python
...
hostname = 'os-122209001573.local'
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if is_ouster:
        ...
        with closing(client.Scans.stream(hostname,7502)) as scans:
            ...
            for scan in scans:
                ...
                ref_val = client.destagger(metadata, ref_field)
                #ref_img = (ref_val / np.max(ref_val) * 255).astype(np.uint8)
                ref_img = ref_val.astype(np.uint8)

                range_field = scan.field(client.ChanField.RANGE)
                range_val = client.destagger(metadata, range_field)
                ...
                #run inference
                dataset = LoadNumpy(numpy=combined_img, path=ROOT, img_size=imgsz, stride=stride, auto=pt and not jit)
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            label = None if hide_labels else (names[c] if hide_conf else f'Loc: {xyz_val[0]:.2f} {xyz_val[1]:.2f} Conf: {conf:.2f}')
                            this_box['probability'] = float(conf)
                            this_box['xmin'] = int(xyxy[0])
                            this_box['xmax'] = int(xyxy[1])
                            this_box['ymin'] = int(xyxy[2])
                            this_box['ymax'] = int(xyxy[3])
                            this_box['Class'] = names[c]
                            this_box['xmin_coord'] = xyz_val[0]
                            this_box['ymin_coord'] = xyz_val[1]
                            # print(this_box)
                            all_boxes['Cube' + str(numBoxes)] = this_box
                            numBoxes += 1
                        ...
                        string_dict = json.dumps(all_boxes)            
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'best.onnx', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'ouster_example_data.pcap', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--metadata-path', type=str, default=ROOT / 'ouster_example_data.json', help='metadata path')
    parser.add_argument('--is-ouster', default=True, action='store_true', help='is ouster')
